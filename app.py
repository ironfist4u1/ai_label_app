...
from engine import run_compliance_audit
from application import render_sidebar, render_form, render_upload, render_results
...
if st.button("Run Verification Audit", type="primary"):
    has_data = any(form_payload.values())
    if not has_data or not uploaded_labels:
        ...
        with st.spinner(f"Running automated evaluations on {brand_name}..."):
            try:
                report = run_compliance_audit(uploaded_labels, form_payload, deep_dive=sidebar_config.deep_dive)
